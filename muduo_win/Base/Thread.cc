...
#pragma pack(pop)
  ...
  return [&]() {
    ...
    } __except (GetExceptionCode() == kMSVCException
...
#if defined(_WIN32)
pid_t gettid() { return uint64_t(GetCurrentThreadId()); }
#endif  // _WIN32
...
struct ThreadData {
  ...
  void runInThread() {
    *tid_ = muduo::CurrentThread::tid();
    ...
    setThreadName(std::this_thread::get_id(),
                  muduo::CurrentThread::t_threadName);
    try {
      func_();
...
void CurrentThread::cacheTid() {
  if (t_cachedTid == 0) {
    ...
    t_tidStringLength =
        snprintf(t_tidString, sizeof t_tidString, "%I64d ", t_cachedTid);
  }
}

bool CurrentThread::isMainThread() {
  if (g_mainTid == 0) {
    g_mainTid = detail::GetMainThreadId();
  }
  return tid() == g_mainTid;
}
...
void Thread::start() {
  ...
  pthread_ =
      std::make_shared<std::thread>(std::thread(&detail::startThread, data));
  if (!pthread_) {
    started_ = false;
